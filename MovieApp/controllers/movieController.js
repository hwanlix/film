...
export async function getMoviesView(req, res) {
  try {
    ...
    console.error('Error in getMoviesView:', error);
    res.render('home-page', { movies: [] });
  }
}
...
export async function searchMovies(req, res) {
  try {
    ...
    const response = await fetch(`${TMDB_BASE_URL}/search/movie?api_key=${TMDB_API_KEY}&language=en-US&query=${encodeURIComponent(query)}`);
    if (!response.ok) {
